...
% Final Project 8.24 - task b

%% initial function
% recives first parameters pf the simulation
% sets the world size accordingly and runs the simulation.
function Project_b(btnDimentionsValue,btnValParticles,btnValInitialTemp,btnValProbability)
 ...
 Run_Model(btnValParticles,btnDimentionsValue,TotalTime,worldsizeX,worldsizeY, btnValInitialTemp,btnValProbability);
   
end

%% Model Generator
function Run_Model(numParticles,Dimentions,time,WorldSizeX,WorldSizeY, InitialTemperature,collesionProbability)
   ...
   %Main VERLET EOM
    for i=2:steps
        ...
        for j=1:Dimentions
          ...
          V = myAndersonThermostat(InitialTemperature,V,numParticles,i,Dimentions,collesionProbability);
...
%% applying Anderson Thermostat in order to maintain a constant temperature
% Anderson Thermostat method is based on hot bath ideology  which means that
% at a given probability  a particle hit a bath and reset its speed.
% Target temperature and collision  probability  are given by the user.
function UpdatedVelocities =  myAndersonThermostat(InitialTemperature,originalVelocity,numParticles,currentStep,Dimentions,collesionProbability)

  for i=1:numParticles

              if rand()<collesionProbability
                  
                  r1 = rand();
                  r2 = rand()*2*pi;
                  
                  originalVelocity(i,1,currentStep) = sqrt(-2*InitialTemperature*log(r1))*cos(r2);
                  
                if Dimentions>1
                   originalVelocity(i,2,currentStep) = sqrt(-2*InitialTemperature*log(r1))*sin(r2);
                end
              end
  end

    UpdatedVelocities = originalVelocity;
