...
function Project_e(btnDimentionsValue,btnValParticles,btnValInitialTemp,btnMvalue)
    ...
    worldsizeXOS=constSigma*(0.03*btnValInitialTemp+5) +constSigma*5/2;
    worldsizeYOS=worldsizeXOS*(sqrt(3)/2);

    Run_Model(2,btnDimentionsValue,TotalTime*4,worldsizeXOS,worldsizeYOS, btnValInitialTemp,1,btnMvalue);
    Run_Model(btnValParticles,btnDimentionsValue,TotalTime,worldsizeX,worldsizeY, btnValInitialTemp,0,btnMvalue);

 
end

%% Model Generator
function Run_Model(numParticles,Dimentions,time,WorldSizeX,WorldSizeY, InitialTemperature,flagOscillator,btnMvalue)
    ...
    %the coefficent factor for the Nose-Hoover Thermostat
    
    coefficientS=zeros(steps,Dimentions);
    ...
    %starting positions for the oscillator simulation
    if flagOscillator==1
        
        ParticlesPositions(1,1,1)=1.99*constSigma;
        ParticlesPositions(1,2,1)=1.99*constSigma;
        ParticlesPositions(2,1,1)=4*constSigma;
        ParticlesPositions(2,2,1)=4*constSigma;    
...
%    Generate Steps
    ...
    % calculate initial Energies and the first Nose-Hoover Thermostat coefficent
   [Etot(1,1), Etot(2,1)] = CalcE(Dimentions,numParticles,M,V,1,ParticlesPositions,WorldSize);
   coefficientS(1,:)= CalculateCoefficent(InitialTemperature,V,M,1,numParticles,Dimentions,TimeLap,btnMvalue);
   ...
   %Main VERLET EOM
    for i=2:steps
        ...
        for j=1:Dimentions
            ...
            coefficientS(i,:)= CalculateCoefficent(InitialTemperature,V,M,i,numParticles,Dimentions,TimeLap,btnMvalue);
            
            V(:,:,i)=myNoseHooverThermostat(coefficientS,V,M,i,F,numParticles,Dimentions,TimeLap);
...
if flagOscillator==1
    ...
    if Dimentions==1
        ...
        P=M(1)*squeeze(V(:,1,:));
        plot(squeeze(ParticlesPositions(1,1,:)),P(1,:));
        hold on;
        xlabel('X(t)');
        ylabel('P(t)');
        grid on;
        ...
        for i=1:steps
                PPlot(i)=sqP(2,i)/max(sqP(2,:));
           
        end

    for i=1:steps

        plot3(squeeze(ParticlesPositions(1,1,i)),squeeze(ParticlesPositions(1,2,i)),squeeze(P(1,1,i)),'Marker','.','color',[0 PPlot(i)  PPlot(i)]);
        hold on;
    end
        ...
        for i=1:steps
            plot(squeeze(ParticlesPositions(1,1,i)),squeeze(P(1,1,i)));
            hold on;
        end
        ...
        for i=1:steps  
            plot(squeeze(ParticlesPositions(1,2,i)),squeeze(P(1,2,i)));
            hold on;
        end
...
%% Calculate the current Nose-Hoover Thermostat coefficent
% the coefficent changes as the velocities change
function CoefficentResult = CalculateCoefficent(InitialTemperature,currentVelocity,M,currentStep,numParticles,Dimentions,TimeLap,btnMvalue)

    coefficientS=zeros(Dimentions,1);
    Kb=1.38*10^-23;
    ...
    for i=1:numParticles
        ...
        if Dimentions==1
            ...
            coefficientS=coefficientS+(M(1)*currentVelocity(i,currentStep)*currentVelocity(i,currentStep)-Dimentions*numParticles*Kb*InitialTemperature*TimeLap)/btnMvalue;

        elseif Dimentions>1
            coefficientS(1)=coefficientS(1)+(M(1)*currentVelocity(i,1,currentStep)*currentVelocity(i,1,currentStep)-Dimentions*numParticles*Kb*InitialTemperature*TimeLap)/btnMvalue;
            coefficientS(2)=coefficientS(2)+(M(1)*currentVelocity(i,2,currentStep)*currentVelocity(i,2,currentStep)-Dimentions*numParticles*Kb*InitialTemperature*TimeLap)/btnMvalue;
        end
    ...
    for i=1:Dimentions
      ...
      coefficientS(i)=coefficientS(i)/M(1);

    end

    CoefficentResult = coefficientS;
    
end


%% applying Nose-Hoover Thermostat in order to maintain a constant temperature
% Nose-Hoover method is based on hot bath ideology  which means that
% as temporary unreal  particle is generated.
% This particle collide with the real particles and reset their velocities.
function NoseHooverVelocity = myNoseHooverThermostat(coefficientS,currentVelocity,M,currentStep,F,numParticles,Dimentions,TimeLap)
  
  NewVelocity=zeros(numParticles,Dimentions);
    
    if Dimentions==1
        for k=1:numParticles
            
            NewVelocity(k,1) = currentVelocity(k,1,currentStep-1) + (F(k,1,currentStep-1)/M(1)-currentVelocity(k,1,currentStep-1)*coefficientS(currentStep-1))*(TimeLap/2) + (F(k,1,currentStep)/M(1)-currentVelocity(k,1,currentStep)*coefficientS(currentStep))*(TimeLap/2);
    ...
    elseif Dimentions>1
        ...
        for k=1:numParticles
          ...
          NewVelocity(k,1) = currentVelocity(k,1,currentStep-1) + (F(k,1,currentStep-1)/M(1)-currentVelocity(k,1,currentStep-1)*coefficientS(currentStep-1,1))*(TimeLap/2) + (F(k,1,currentStep)/M(1)-currentVelocity(k,1,currentStep)*coefficientS(currentStep,1))*(TimeLap/2);
          NewVelocity(k,2) = currentVelocity(k,2,currentStep-1) + (F(k,2,currentStep-1)/M(1)-currentVelocity(k,2,currentStep-1)*coefficientS(currentStep-1,2))*(TimeLap/2) + (F(k,2,currentStep)/M(1)-currentVelocity(k,2,currentStep)*coefficientS(currentStep,2))*(TimeLap/2);
...
NoseHooverVelocity =  NewVelocity;
...
%initial volocities are based on the equation:
